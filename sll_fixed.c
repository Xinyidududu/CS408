...
char *fgets_enhanced(FILE *f)
{
    ...
    if (fgets(s, chunksize+1, f) == NULL)
    {
        ...
        while (nPos == NULL)
        {
            ...
            temp = realloc(s, buffersize); //here is the problem
            if (temp == NULL)
            {
...
void delete_all()
{
    struct node *temp; // Fix for Q2 (b)
    while (p != NULL)
    {
        temp = p;            // Store the current node // Fix for Q2 (b)
        p = p->next;         // Move to the next node // Fix for Q2 (b)
        
        free(temp->str);     // Free the string // Fix for Q2 (b)
        free(temp);          // Free the node itself // Fix for Q2 (b)
    }
}
...
void edit(long int oldNum, long int newNum, char* name)
{
    struct node *temp = p; //Fix for Q2 (b)
    while (temp != NULL)
    {
        if (temp->data == oldNum)
        {
            temp->data = newNum;
            free(temp->str); // Free the old string memory
            temp->str = name; // Assign the new string
            return;
        }
        temp = temp->next; // Fix for Q2 (b)
    }
    printf("ELEMENT %ld NOT FOUND\n", oldNum);
}
